...
# Output: RALPH-66ff1a2b-a3f7b2c94e1d08f6a2b3c5d7e1f4a09b2c4d6e
```

- **RALPH-** prefix prevents false matches against hex strings in code output
- **8-char epoch hex** provides structural temporal uniqueness (decodable: `printf '%d\n' 0x66ff1a2b`)
- **40 hex chars** from `/dev/urandom` = true OS randomness, zero LLM bias
...
137 tests across 10 suites, all passing:
...
| test-passphrase-v2.sh        | 25    | v3 format, epoch validation, cross-session, YAML round-trip          |
...
| test-session20-fixes.sh      | 17    | Hook matcher, sandbox nvme-fast paths, command sync                  |
